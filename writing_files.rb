...
File.open('TextFiles/Employees2.txt', 'w') do |file|
  file.write('Juan Carlos, Accounting') # Write something to the file, overwriting the existing content - Es decir que
  # si el archivo ya existe, se sobreescribira el contenido existente.
end

File.open('TextFiles/nuevoArchivoPrueba.txt', 'w') do |file|
  file.write('Este es una prueba para la creación de un nuevo archivo') # Write something to the file, overwriting the
  # existing content - Es decir que si el archivo ya existe, se sobreescribira el contenido existente, si no existe se
  # creara con el contenido y en la ruta indicada.
end
